...
export function buildRecipeInstructions(locale: SupportedLocale = "de-DE", excludeModes: PromptModeType[] = []): string {
  ...
  return [
    ...
    `STEAMING PREHEAT CHOICE: For every steaming step (mode type 'steam'), you MUST instruct the user in the step description whether to preheat the pot based on starting liquid temperature. Append the exact localized phrase: if starting liquid is cold/room-temperature, append "${localeGuidance.accessoryPhrases.preheatSteamingCold}"; if liquid is already hot/boiling, append "${localeGuidance.accessoryPhrases.preheatSteamingHot}". The entire step description (including this instruction) must still be at or below 240 characters.`,
    `SCRAPE DOWN RULE: For any high-speed chopping step (speed 4 or above, or using turbo), the step description MUST instruct the user to scrape down the sides of the jug using the spatula by appending the exact phrase: "${localeGuidance.accessoryPhrases.spatulaScrape}".`,
...
3. Step description: Must instruct the user to prepare the ingredient and specify the net weight to weigh. Format this exact instruction using the localized template: "${localeGuidance.accessoryPhrases.grossNetWeightTemplate}" by substituting {ingredient} with the name of the ingredient and {net} with the net weight. Ensure the entire step description (including this instruction) is at or below 240 characters.`,
    `HOT PUREEING SAFETY WARNING: When pureeing hot contents (temperature > 60 °C, or using automatic 'puree' or 'smoothie' mode, or manualCooking speed >= 4 on hot ingredients), the step description MUST instruct the user to wait about 10 seconds after mixing stops before opening the lid by appending the exact localized phrase: "${localeGuidance.accessoryPhrases.hotPureeingSafety}". The entire step description (including this warning) must be at or below 240 characters.`,
    `PRE-CUTTING INGREDIENTS: For any step chopping, pureeing, or using turbo on solid foods (speed >= 4, 'puree', 'smoothie', 'turbo', or 'foodProcessor' mode), the step description MUST instruct the user to pre-cut ingredients into pieces of approximately 3-4 cm by appending/including the exact localized phrase: "${localeGuidance.accessoryPhrases.preCutIngredients}". The entire step description must be at or below 240 characters.`,
    `SIMMERING BASKET LIQUID LIMIT: Boiling or steaming using the simmering basket requires at least 500 ml of water or liquid. The step description MUST instruct the user to add at least 500 ml of liquid using the exact localized phrase: "${localeGuidance.accessoryPhrases.minLiquidSimmeringBasket}". The entire step description must be at or below 240 characters.`,
    ...
    "CHOP-SCRAPE-SAUTÉ SEQUENCE: When a recipe calls for sautéing aromatics (onions, garlic, carrots), break it into three distinct steps: 1. Add them whole or halved and chop (speed 5 for 5 seconds); 2. Instruct the user to scrape down the sides using the spatula; 3. Add oil/butter and run the 'roast' (Anbraten) mode.",
    `LID DRIP EMULSION TRICK: For emulsions (like mayonnaise or hollandaise), do not instruct the user to add the oil directly to the jug. Instead, instruct them to leave the measuring cup inserted and pour the oil slowly onto the lid by appending/including the exact localized phrase: "${localeGuidance.accessoryPhrases.emulsionOilDrip}". The entire step description must be at or below 240 characters.`,
    ...
    `FAT-FREE WHIPPING: If a recipe requires whipping egg whites using the butterfly whisk, explicitly state in the description that the jug and whisk must be clean, dry, and fat-free by appending the exact localized phrase: "${localeGuidance.accessoryPhrases.cleanDryFatFree}". The entire step description must be at or below 240 characters.`,
    ...
    `MOISTURE REDUCTION: For steps involving reducing a liquid, thickening a sauce, or boiling jam, instruct the user to remove the measuring cup using the phrase: "${localeGuidance.accessoryPhrases.removeMeasuringCup}" and place the simmering basket on the lid as a splash guard using the phrase: "${localeGuidance.accessoryPhrases.useBasketAsSplashGuard}". The entire step description must be at or below 240 characters.`,
    `STEAMER AIRFLOW: Whenever transitioning to a step that uses the external steamer attachment, explicitly instruct the user to remove the measuring cup using the phrase: "${localeGuidance.accessoryPhrases.removeMeasuringCup}" before placing the steamer attachment on top. The entire step description must be at or below 240 characters.`,
    `RAPID COOL-DOWN: If a step requires the contents of the jug to cool down (e.g., waiting for the temperature to drop below 80 °C before adding eggs, or 37 °C for yeast), instruct the user to remove the mixing bowl from the base and remove the measuring cup using the phrase: "${localeGuidance.accessoryPhrases.removeMeasuringCup}". The entire step description must be at or below 240 characters.`,
    `CONTINUOUS FEEDING: For steps requiring the gradual addition of ingredients while the machine is running (adding flour to dough, dropping ice, etc.), instruct the user to remove the measuring cup using the phrase: "${localeGuidance.accessoryPhrases.removeMeasuringCup}" and drop ingredients through the opening in the lid. The entire step description must be at or below 240 characters.`,
    "",
...
export function accessoryHardwareRules(locale: SupportedLocale = "de-DE"): string {
  ...
  return [
    ...
    `- ${terms.measuringCup}: Mention when it should be kept inserted to reduce splashing, or removed for evaporation during reduction/roasting/thickening steps (where the ${terms.simmeringBasket} should be used instead as a splash guard: "${phrases.useBasketAsSplashGuard}"). Instruct the user to remove the measuring cup ("${phrases.removeMeasuringCup}") for steaming, rapid cool-down, continuous feeding of ingredients, or when pouring oil slowly onto the lid for emulsions: "${phrases.emulsionOilDrip}".`,
