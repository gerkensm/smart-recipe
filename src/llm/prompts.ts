...
export function accessoryHardwareRules(locale: SupportedLocale = "de-DE"): string {
  ...
  return [
    ...
    `- ${terms.measuringCup}: Mention when it should be kept inserted to reduce splashing, or removed for evaporation during reduction/roasting steps (where the ${terms.simmeringBasket} should be used instead as a splash guard: "${phrases.useBasketAsSplashGuard}").`,
